...
var Survey = {
	...
	load: function(id) {
		...
		if (id == "new") {
			...
			return db.get(id).then(function(doc) {
				Survey.current = doc
				console.log("survey.load: returned :id " + doc._id + " rev: " + doc._rev)
				m.redraw()
			}).catch(function(err) {
				console.log(err)
			})
	...
	save: function() {
		...
		return db.put(Survey.current).then(function(response) {
			console.log("survey.save: ok " + response.ok + " rev: " + response.rev)
		}).catch(function(err) {
			console.log(err)
		})
	}
