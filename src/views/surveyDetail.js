...
module.exports = {
	...
	view: function() {
		console.log("surveyDetail.view: " + Survey.current._id + ", " + Survey.current.date)
		return m(".w3-card w3-row-padding w3-padding", [
			m(".w3-container w3-black", [
				m("header.w3-container w3-black", 
					m("h4", ((Survey.current.mdmapid) ? "MDMAP ID #" + Survey.current.mdmapid + " - " : "") + Survey.current.name),
					m("button.w3-button w3-black w3-padding-large w3-large w3-right", { href: "/survey/edit/" + Survey.current._id, oncreate: m.route.link }, "Edit")
				),
			]),
			m(".w3-container", [
